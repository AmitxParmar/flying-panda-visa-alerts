import type { AuthErrorCode, ApiErrorResponse, AuthUserResponse, ApiSuccessResponse } from "@/types";
...
// Internal refresh token function to avoid circular dependency
const refreshAuthToken = async (): Promise<ApiSuccessResponse<AuthUserResponse>> => {
  // Use a fresh axios instance to avoid interceptors
  const response = await axios.post<ApiSuccessResponse<AuthUserResponse>>(
